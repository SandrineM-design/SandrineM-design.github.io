function Skills() {
  ...
  return (
    <div className="flex flex-col flex-1 place-items-center">
      ...
      <h3 className="mt-2.5 font-bold text-1x1 text-emerald-600">Frontend :</h3>
      <ul className="flex flex-col-3 gap-10 mt-2 mb-2 list-disc">
        {frontend.map((frontend, index) => (
          <li key={index}>{frontend}</li>
        ))}
      </ul>
      <h3 className="mt-2.5 font-bold text-1x1 text-emerald-600">Backend :</h3>
      <ul className="flex flex-col-1 gap-10 mt-2 mb-2 list-disc">
        {backend.map((backend, index) => (
          <li key={index}>{backend}</li>
        ))}
      </ul>
      <h3 className="mt-2.5 font-bold text-1x1 text-emerald-600">Outils :</h3>
      <ul className="flex flex-col-3 gap-10 mt-2 mb-2 list-disc">
